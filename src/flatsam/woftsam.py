...
import logging
from flatsam.utils.geom import H_compose
import flatsam.utils.vis as vu
from flatsam.utils.timing import time_measurer, general_time_measurer

logger = logging.getLogger(__name__)
logging.getLogger("numexpr.utils").setLevel(logging.WARNING)

class WOFTSAM():
    # YAOFTrackerSingleControl from WOFT:YAOF_tracker_single_control.py
    ...
    def track(self, input_img, robust_H2init=None, debug=False, img_identifier=None):
        ...
        H_global_cur2init, global_H_success = self.estimate_H_with_prewarp(prewarp_H, input_img)
        ...
        if global_H_success:
            ...
            H_sam_cur2init, sam_H_success = self.estimate_H_with_prewarp(robust_H2init, input_img)
            if sam_H_success:
                print('WOFT failed, saved by SAM-H')
                H_cur2init = H_sam_cur2init
                self.lost = False
                self.N_lost = 0
            else:
                print('WOFT and SAM-H failed')
                self.lost = True
                self.N_lost += 1

                if self.C.fallback_robust:
                    H_cur2init = robust_H2init
                    if self.C.fallback_nolost:
                        self.lost = False
                        self.N_lost = 0
                elif self.C.no_local_H:
                    H_cur2init = H_sam_cur2init
                else:
                    ...
                    try:
                        H_flow = self.C.H_estimator(
                            ...
                            weights)
                        H_flow = einops.rearrange(to_numpy(H_flow.to(torch.float32)), '1 r c -> r c', r=3, c=3)
                        H_local_cur2init = H_compose(H_flow, self.prev_H2init)
                    except Exception:
                        logger.exception("local flow H-estimation failed")
        ...
        if False and debug:
            ...
            pw_flow = einops.rearrange(to_numpy(pw_flow),
                                       'xy H W -> H W xy', xy=2)
            pw_colors = vu.cv2_colormap(einops.rearrange(pw_weights, '1 H W -> H W'), vmin=0, vmax=1, do_colorbar=False)
            ...
            vis_pw_flow = vu.vis_flow_align(pw_flow, self.template_img, prewarped_input, grid_sz=5,
            ...
            local_flow = einops.rearrange(to_numpy(local_flow),
                                          'xy H W -> H W xy', xy=2)
            vis_local_flow = vu.vis_flow_align(local_flow, self.prev_img, input_img, grid_sz=30)
            cv2.imshow("cv: vis_local_flow", vis_local_flow)

            local_flow = vu.vis_alignment_plain(self.prev_img, input_img)
            prewarp = vu.vis_alignment_plain(self.template_img, prewarped_input)
            afterwarped_input = cv2.warpPerspective(input_img, H_global_cur2init,
                                                    (input_img.shape[1], input_img.shape[0]),
                                                    flags=cv2.INTER_LINEAR)
            afterwarp = vu.vis_alignment_plain(self.template_img, afterwarped_input)
            ...
            if True:
                composition = vu.tile(
                    vu.griddify(
                        vu.name_fig([local_flow, prewarp, afterwarp],
        ...
        if self.C.downscale_inputs:
            ...
            H_cur2init = H_compose(H_downscale, H_cur2init, H_upscale)
    ...
    def estimate_H_with_prewarp(self, prewarp_H, input_img):
        if prewarp_H is None:
            return None, False
        ...
        # global_flow_timer = general_time_measurer("global_flow_timer", 'ms')
        ...
        H_global_cur2init = H_compose(prewarp_H, np_H_prewarped2init)
        ...
        return H_global_cur2init, global_H_success
